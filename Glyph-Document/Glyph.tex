...
{The \textcolor{blue}{\textsf{glyphML}}
...
%%%%%%%%%%%%%%%%%%%%%%%%
\clearpage
\S{The glyphML notation}

The \Scala{glyphML} notation is a dialect of
...
functions to transform \textbf{glyphML} elements to glyphs when such
...
\Scala{glyphML} expressions as \Scala{Glyph}s, we will not explore
that role in detail here.}

The following example defines a couple of glyphs. 

\verbatimfamily{\sffamily}
\verbatimsize{\footnotesize\color{blue}}
\begin{verbatim}
val style: StyleSheet = StyleSheet(fontScale=0.6f)
import glyphML._
val language = Translator().withPackage(DecorativeExtension)(style)
import language._
val gxml1: Glyph=
...
val gxml2: Glyph =
  <div width="27em" align="justify">
    <macro tag="roundframe">
      <frame enlarge="30" radius="30" fg="black.4.dashed(5,5)">
        <?body?
      </frame>
    </macro>
    <roundframe>
...
Both glyphs use framing; the second shows the use of a macro definition to
ensure consistent framing in a context. The hyphenation in that example appears
at discretionary points: indicated by the presence of underscores in the text.
...
\Scala{tests.GlyphBook}, and the smaller
...
These demonstrate sophisticated use of \Scala{Exam} 
...
in \Scala{glyphML}, a notation reminiscent of (but different from
...
of the \Scala{tests.GlyphBook} example. They 
...
\clearpage
\SS{Overview of the code behind  Figures \ref{DynamicResizing3} -- \ref{DynamicResizing5}}
The code of \Scala{ResizeableWindowTest} is worth studying in detail, and here we give an overview
of its structure in order to facilitate this.

We first define a style sheet, and a \Scala{glyphML.Translator}. The latter will be
used when we want to render \Scala{glyphML} source text as \Scala{Glyph}s
\let\scalasize\scriptsize
\begin{scala}
...
The most important feature is that the application's top-level \Scala{GUI:
Glyph} is a \Scala{Resizeable}, whose functional argument is invoked
whenever the window size changes, and whenever the (text) font
size changes.
That function maps the current context (which starts off as the original
`style`) into an appropriate GUI Glyph.
...
val GUI: Glyph = styled.Resizeable {
  ...
  case context: StyleSheet if context.containerWidth < 25*context.emWidth =>
    // the window size is smaller than 25em -- the GUI reduces to a red-edged button
    lazy val wider: Glyph =
       styled.TextButton(f"""...""") {
      _ =>
...
When the window is (or has become) too narrow it defines the GUI as a
red-edged button whose text explains what has happened, and what to do
about it.\footnote{Pressing the button forces the window to be an adequate
size.  This can also be done by dragging an edge of the window horizontally
to an appropriate size.}
When the window is wide enough, the GUI is generated by \Scala{guiSpec(context)}.
\clearpage

The \Scala{guiSpec} function first declares a glyph containing a
few buttons , then the glyph containing the text, and then yields
a column with the buttons placed over the text. The whole is edged in
black.
...
\begin{comment}
(whose implementation details we have elided), places
them in a fixed-size row together with a \Scala{Label} that shows
the current width of the text (in ems), and underlined with a thick
black line. The typographic details of the buttons depend implicitly
on the \Scala{style} parameter; and the width of the row in
which they are placed depends explicitly on it
{
      ...
      Col(align = Center)(
        FixedSize.Row(style.containerWidth).Mid(
          wider, em, narrower, hFill(),
          Label(s"[${(style.containerWidth/style.emWidth).toInt}ems]"), hFill(),
...
The text is specified in \Scala{glyphML}, and its layout as a glyph is determined using the
translator defined earlier, whose  style parameter embodies the most important detail
of the layout, namely the width of the container in which the material is to
fit. This is used to define the outermost \verb/width/
attribute.\footnote{Notice the macro definitions that are used to provide a semblance of readability to
the source language. We disable warnings about macro redefinitions, because they
are made \textit{whenever} the text is laid out.}
...
\begin{scala}
  ...
  <div width="container.width" align="justify" macrowarning="off">
     ...
     <macro tag="sup">
      <row alignment="top">
        <strut/>
         <span textforeground="red" fontscale="0.6">
          <b><?body?></b>
         </span>
       </row
     </macro>
     ...
     <p>
       All De Rerum Natura bases are free of mule_sing<sup>&ddagger;</sup>
       and are ...
     </p>
     <p>
       <i>Try changing the <bi>width</bi> ...</i>
     </p>
     <strut/>
     <centered>
...
of \Glyph then \Scala{tests.GlyphBook} is a
good place to start. 
...
and \Scala{styled.windowdialogues} packages show how to use the
\Glyph's basic features to build popup dialogues and menus.

The package \Scala{glyphML} was a latecomer to this party. It
integrates the \Scala{scala.xml}  notation embedded (in Scala 2)
with styles, and makes it fairly straightforward to mix styled
texts with glyphs in a way that may be helpful. It has been revised
a couple of times and is used throughout much
