...
import org.sufrin.glyph.GlyphTypes.{Font, PaintMode, Path, PathFillMode, Scalar}
import org.sufrin.glyph.Shape.{GlyphPath, composite, rect, superimposed}
...
object Shape {
  ...
  class GlyphPath extends io.github.humbleui.skija.PathBuilder() {
    def toPath: Path = snapshot()
...
class PathShape(override val fg: Brush, absolute: Boolean = true) extends Shape {
  ...
  def draw(fg: Brush, surface: Surface): Unit = if (absolute) {
    surface.drawPath(fg, path.toPath)
  } else {
  ...
  def addPathShape(shape: PathShape, x: Scalar, y: Scalar): PathShape = {
    path.addPath(shape.path.toPath, x-shape.w/2, y-shape.h/2)
    this
  }

  def addPathShape(shape: PathShape, pos: (Scalar, Scalar)): PathShape = {
    path.addPath(shape.path.toPath, pos._1-shape.w/2, pos._2-shape.h/2)
    this
  }
