...
import org.sufrin.glyph.Shape.GlyphPath
...
/** A collection of static `Glyph` constructors */
  ...
  object Shaded  {
    ...
    def shadingPaths(w: Scalar, h: Scalar, delta: Scalar): ShadingPaths = {
      import org.sufrin.glyph.Shape.GlyphPath
      val bottomRight = new GlyphPath() {
        moveTo(0f, h)
        lineTo(w, h)
        lineTo(w, 0f)
        lineTo(w + delta, delta)
        lineTo(w + delta, h + delta)
        lineTo(delta, h + delta)
        //
 ...
 *
 */
class Path(override val fg: Brush, override val bg: Brush, strictContains: Boolean, val thePath: GlyphPath = new GlyphPath) extends Glyph {
  hostShape =>
  ...
  var path = thePath.toPath

  locally {
    if (!normalize) thePath.moveTo(0, 0)
    path = thePath.toPath
  }
  ...
  def addRect(x: Scalar, y: Scalar, w: Scalar, h: Scalar): Path = {
    ...
    thePath.addRect(makeXYWH(x, y, w, h)); this
  }

  def addOval(x: Scalar, y: Scalar, w: Scalar, h: Scalar): Path = {
    import io.github.humbleui.types.Rect.makeXYWH
    thePath.addOval(makeXYWH(x, y, w, h)); this
  }

  def addOval(pos: (Scalar, Scalar), dim: (Scalar,Scalar)): Path = {
    import io.github.humbleui.types.Rect.makeXYWH
    thePath.addOval(makeXYWH(pos._1, pos._2, dim._1, dim._2)); this
  }

  def addArc(x: Scalar, y: Scalar, w: Scalar, h: Scalar, startAngle: Scalar, sweepAngle: Scalar): Path = {
    import io.github.humbleui.types.Rect.makeXYWH
    thePath.addArc(makeXYWH(x, y, w, h), startAngle, sweepAngle); this
  }

  def addArc(pos: (Scalar, Scalar), dim: (Scalar,Scalar), startAngle: Scalar, sweepAngle: Scalar): Path = {
    import io.github.humbleui.types.Rect.makeXYWH
    thePath.addArc(makeXYWH(pos._1, pos._2, dim._1, dim._2), startAngle, sweepAngle); this
  }

  def addCircle(x: Scalar, y: Scalar, r: Scalar): Path = {
    thePath.addCircle(x, y, r); this
  }

  def addCircle(pos: (Scalar, Scalar), r: Scalar): Path = {
    thePath.addCircle(pos._1, pos._2, r); this
  }

  def addPath(shape: Path, x: Scalar, y: Scalar): Path = {
    thePath.addPath(shape.path, x-shape.w/2, y-shape.h/2)
    this
  }

  def addPath(shape: Path, pos: (Scalar, Scalar)): Path = {
    thePath.addPath(shape.path, pos._1-shape.w/2, pos._2-shape.h/2)
    this
  }

  override def toString: String = s"Path($fg)(${thePath.getVerbs.toSeq.mkString(",")})"


  /** A copy of this glyph that shares the same thePath */
  def copy(fg: Brush=fg, bg: Brush=bg): Glyph = new Path(fg, bg, strictContains, thePath)
}
