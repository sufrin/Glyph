...
class Etcetera(implicit val style: BookSheet, implicit val translator: glyphML.Translator)  {
  ...
  import Brushes._
    ...
    Page("Path"){
      ...
      val b1Up    = p2()
      val b1Down  = p3.rotated(1)
      val b1Hover = p2.rotated(2)

      val b1 = new unstyled.reactive.RawButton(b1Up, b1Down, b1Hover, fg=transparent, bg=transparent, { _=> }) {
      ...
      Col(align=Center, skip=20)(
        <div width="80em" align="justify">
          <p>
             This example shows shapes and buttons constructed from
              <b>Path</b> glyphs. The glyphs all have
              <tt>fillMode(PathFillMode.EVEN_ODD)</tt>, and <tt>strictContains = true</tt>.
          </p>
        </div>, ex,
        Row(Label("P1: Triangle + Circle") above p1().framed(), em, em, em, em, em,  Label("P1 + 3 Circles") above p0().framed()),
        ex, ex,
        <div width="80em" align="justify">
          <p>
            A RawButton using three distinctly-brushed copies of the path P1; one rotated. Notice
            that the button reacts to hovering only when the cursor enters the shape proper (not
            just its bounding box).
          </p>
        </div>, ex,
        b1.framed(), ex,
        Row(Label("Up")    above b1Up(),    em,
            Label("Down")  above b1Down(),  em,
            Label("Hover") above b1Hover(), em
        ),
        ex, ex,
