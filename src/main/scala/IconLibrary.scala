...
object IconLibrary {
  ...
  def SETTINGS(brush: Brush = Brushes.darkGrey): Glyph = {
    ...
    val plank: GlyphShape = rect(18, 3)(brush)
  ...
  def CROSS45(diam: Scalar = 28f, down: Brush = crossBrush, up: Brush = crossBrush, fill: Brush = redFill): Glyph = {
    import GlyphShape._
    val halfDiam: Float = (Math.sin(Math.PI * 0.25) * diam / 2).toFloat
    val downBar: GlyphShape = line(-halfDiam, -halfDiam, halfDiam, halfDiam)(down(width = diam / 8f))
    val upBar: GlyphShape = line(-halfDiam, halfDiam, halfDiam, -halfDiam)(up(width = diam / 8f))
  ...
  def CROSS(diam: Scalar = 28f, ns: Brush = crossBrush, ew: Brush = crossBrush, fill: Brush = redFill): Glyph = {
    import GlyphShape._
    val halfDiam: Float = diam / 2
    val nsBar: GlyphShape = line(0, -halfDiam, 0, halfDiam)(ns(width = diam / 8f))
    val ewBar: GlyphShape = line(-halfDiam, 0, halfDiam, 0)(ew(width = diam / 8f))
